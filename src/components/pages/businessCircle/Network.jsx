    ...
    <div className="space-y-6">
      ...
      {!selectedCircleId ? (
        <div className="rounded-2xl border border-hairline bg-white p-16 text-center">
          ...
          <p className="text-[14px] text-muted">Select a circle to manage circle meetings.</p>
        </div>
      ) : (
...
function SectorTeamsPanel({ circleId }) {
  return (
    <div className="rounded-2xl border border-hairline bg-white p-8 text-center">
      <p className="text-[13.5px] text-muted">Select a circle to manage its inter-circle network.</p>
    </div>
  );
}