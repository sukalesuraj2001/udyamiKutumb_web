...
import { ArrowLeft, MapPin, Phone, Mail, Briefcase, User, Calendar, 
Building, Globe, Users, Award, Heart, Star, Clock, Link, Edit, Camera, Smartphone,
Download, Video, Printer, Share2, CheckCircle, MessageCircle, UserCheck,
} from "lucide-react";

import { useNavigate, useParams } from "react-router-dom";
import { CP_INITIAL_MEMBERS } from "./ChannelPartner.jsx";
...
export default function CpMemberDetails({ member: propMember, onBack, onEdit, onSuspend }) {
  const [activeTab, setActiveTab] = useState("overview");
  const navigate = useNavigate();
  const { id } = useParams();

  // Use prop member if provided, otherwise find from CP_INITIAL_MEMBERS
  const member = propMember || CP_INITIAL_MEMBERS.find(m => m.id === id);
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        >
          ...
          <span className="text-[13.5px] font-medium">Back To CP Dashboard</span>
        </button>
