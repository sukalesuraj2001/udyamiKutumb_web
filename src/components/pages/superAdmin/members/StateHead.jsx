  ...
  return (
    <div className="bg-[#F8FAFC] -m-6 p-6 min-h-full space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[20px] font-bold text-[#0F172A] leading-tight">State Heads</h1>
          <p className="text-[12px] text-[#94A3B8] mt-0.5">
            Manage all state-level heads · {STATE_HEAD_DATA.length} records
          </p>
        </div>
      </div>
      ...
      {/* ── TABLE ────────────────────────────────────────────── */}
      <HeadTable
        title="State Head List"
        subtitle="All states · Platform-level representatives"
        icon={Users}
        type="state"
        data={STATE_HEAD_DATA}
        onView={handleView}
        onEdit={handleEdit}
